...
class MyListData : AppCompatActivity(), RecyclerViewAdapter.datalistener {
    ...
    private var dataTeman = ArrayList<data_buku>()
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        supportActionBar!!.title = "DataBuku"
    ...
    private fun GetData() {
        ...
        getReference.child("Admin").child(getUserID).child("DataBuku")
            .addValueEventListener(object : ValueEventListener {
                override fun onDataChange(datasnapshot: DataSnapshot) {
                    if (datasnapshot.exists()) {
                        dataTeman.clear()
                        for (snapshot in datasnapshot.children) {
                            val teman = snapshot.getValue(data_buku::class.java)
    ...
    override fun onDeleteData(data: data_buku?, position: Int) {
        val getUserID: String = auth?.getCurrentUser()?.getUid().toString()
        val getReference = database.getReference()
